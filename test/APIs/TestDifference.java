...
public class TestDifference {
  ...
  /*
   * Testing strategy: 1.setTrackNumDiff: 1）输入：>0,==0,<0 2.setObjNumDiff： 1）tracknumber:
   * 是否超过了目前得最大下标：是，否。 3.setObjDetailDiff: 1）tracknumber: 是否超过了目前得最大下标：是，否。
   * 2）objdetaildiff：是否为空串：是，否。 4.appendObjDetailDiff: 1）objdetaildiff：是否为空串：是，否。 5.getObjNumDiff:
   * 1）tracknumber: 是否超过了目前得最大下标：是，否。 5.getObjDetailDiff： 1）tracknumber: 是否超过了目前得最大下标：是，否。
   */
  @Test
