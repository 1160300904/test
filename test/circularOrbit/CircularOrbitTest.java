...
public class CircularOrbitTest {
  /*
   * Testing strategy: (1)setCentralObj() : 输入的物体对象是否为空 (2) boolean addInsideTrack(double radius)
   * ①输入的轨道半径是否<0 ②轨道半径是否与系统中已有轨道的半径相同 ③输入轨道半径是否比已有轨道的半径都小或都大 (3)void deleteTrack(int tracknum)
   * ①输入轨道号码是否超过了已有的轨道数目 (4)boolean addPhyToTrack(E physicalObject, double theta, int tracknum)
   * ①输入的物体是否为null ②输入的角度为0，360,90,180,270,>360,<0 ③输入轨道编号是否超过了已有轨道数目 (5) boolean removePhy(E
   * physicalObject, int tracknum) ①输入的物体是否为null ②输入轨道编号是否超过了已有轨道数目 (6)boolean addPhyRelation(E
   * physicalObject1, E physicalObject2) ①两个物体是否相同 ②两个轨道物体是否为null ③两个轨道物体是否已经存在了轨道系统上 (7)boolean
   * transit(E object, double theta, int sourtracknum,int tartracknum) ①输入的物体是否为null
   * ②输入的角度为0，360,90,180,270,>360,<0 ③Sourtracknum，和tartracknum是否超过了已有的轨道数目 (8)int NumOnTrack(int
   * tracknum) ①tracknum是否超过了已有的轨道数目 ②该轨道上是否有物体 (9)double getPhysicalDistance( E e1, E e2)
   * ①这两个物体是否有不存在于轨道系统中的 ②两个物体是否在一条水平或竖直线上 ③两个物体是否在一条轨道上 ④两个物体的角度是否相同 ⑤两个物体的位置是否重合 (10) int
   * getLogicalDistance(E p1, E p2) ①两个物体间是否不存在逻辑关系 ②两个物体的逻辑关系是否为所有物体对中最远的 ③两个物体是否相同
   */
  @Test
