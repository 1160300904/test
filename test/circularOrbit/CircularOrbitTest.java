...
public class CircularOrbitTest {
	 ...
	 * 	(1)setCentralObj() :	输入的物体对象是否为空
		...
		(7)boolean transit(E object, double theta, int sourtracknum,int tartracknum) 
			①输入的物体是否为null
			②输入的角度为0，360,90,180,270,>360,<0
			③Sourtracknum，和tartracknum是否超过了已有的轨道数目
		(8)int NumOnTrack(int tracknum)
			①tracknum是否超过了已有的轨道数目
			②该轨道上是否有物体
