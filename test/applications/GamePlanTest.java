...
public class GamePlanTest {
  ...
  /*
   * Testing strategy 1.RandomPlan ①List<List<Athlete>> plan(List<Athlete> athletes,int tracknum)
   * 1)输入运动员列表是否有重复运动员 2)输入运动员列表中是否正好为轨道数目的整数倍 3)轨道数目是否合法，在【4，10】之间 2.sortedPlan
   * ①List<List<Athlete>> plan(List<Athlete> athletes, int tracknum) 1)输入运动员列表是否有重复运动员
   * 2)输入运动员列表中是否正好为轨道数目的整数倍 3)轨道数目是否合法，在【4，10】之间 4)运动员是否已经以最好成绩排好序
   */
  @Test
  public void testRandomPlan() {
    ...
    assertEquals(7, retlist.get(0).size());
    assertEquals(3, retlist.get(1).size());
    ...
    athnum = 14;
    List<Athlete> alist1 = new ArrayList<>();
    List<List<Athlete>> retlist1 = new ArrayList<>();
    for (int i = 1; i <= athnum; i++) {
  ...
  public void testSortedPlan() {
    ...
    List<Athlete> retlist0 = retlist.get(0);
    List<Athlete> retlist1 = retlist.get(1);
    ...
    athnum = 2;
    List<Athlete> alist1 = new ArrayList<>();
    List<List<Athlete>> retlist11 = new ArrayList<>();
    for (int i = 1; i <= athnum; i++) {
