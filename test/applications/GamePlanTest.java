...
public class GamePlanTest {
    ...
    /*
     * Testing strategy 1.RandomPlan ①List<List<Athlete>> plan(List<Athlete>
     * athletes,int tracknum) 1)输入运动员列表是否有重复运动员 2)输入运动员列表中是否正好为轨道数目的整数倍
     * 3)轨道数目是否合法，在【4，10】之间 2.sortedPlan ①List<List<Athlete>> plan(List<Athlete>
     * athletes, int tracknum) 1)输入运动员列表是否有重复运动员 2)输入运动员列表中是否正好为轨道数目的整数倍
     * 3)轨道数目是否合法，在【4，10】之间 4)运动员是否已经以最好成绩排好序
     */
    @Test
