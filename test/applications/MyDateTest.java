...
public class MyDateTest {
    ...
    /*
     * Testing Strategy: 1.与加有关的的函数： 1)输入：0，>0; 2)输入的时间：是否比最大值大，即，是否需要进位。
     * 2.与阶段有关的函数： 1）被截断的时间在指定条件下还是否需要被截断，例，若时间为1999:01:01 13:00:00 而要求截断到分钟。
     * 3.observer方法： 1）获取的时刻类型的时间值：0, 非0. 4.getmin方法： 1）哪个时间点最靠前：第一个，第二个，第三个
     * 2）三个输入时间相同的个数：一个，两个，三个 5.TruncBaseOnSplit： 1）截断的策略： Hour|Day|Week|Month
     * 2）是否以及满足截断的要求了：方法调用之后，返回的MyDate对象和原来一样。 6.addBaseOnSplit: 1）增加的策略：
     * Hour|Day|Week|Month 2）原日期在增加后是否需要进位 3）输入的策略是否不在 Hour|Day|Week|Month 中。
     * 7.equals: 1）两者相等，不相等 2）输入的两个对象是否为同一个对象 3）两个对象是否为同一个类型。
     */
