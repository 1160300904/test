...
public class TrackGameTest {
     ...
     * 5）交换的两个运动员是否为同一个 2.islegalOneGroup 1）c:中心物体是否是null
     * 2）c中轨道数是否为非法数字（是否在[4,10]之间） 3）c中每个跑道上的运动员数量是否是合法的：每条跑道上只能有一名运动员
     * 3.getAthleteNum 1）tracknum是否合法，是否在[4,10]区间内 2）输入中是否存在不包含任何物体的轨道
     * 4.noSameAthInBothOrbits 1）两轨道上是否有相同的物体： 2）两轨道上是否存在其中没有物体的情况
     */
    @Test
    public void testInitWithFile() throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
        ...
        /*
         * �˴�����assert�ⲻ�� c2.removePhy(a1, 1);c2.addPhyToTrack(a6, 1,
         * 1);c2.addPhyToTrack(a11, 10, 1); assertFalse(t1.isLegal(clist));
         */
        ...
        /*
         * t1.ArrangeGame(new SortedPlan(), 5); List<List<Athlete>>
         * gameplan=t1.getGamePlan(); List<Athlete> gp=gameplan.get(0);
    ...
    public void testSetAndGetGamePlan() throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
        /*
         * ApplicationFactory tf=new TrackGameFactory(); TrackGame
         * t1=(TrackGame)tf.getApplication(); File f1=new
         * File("src\\inputfile\\TrackGameForTest.txt"); t1.initFromFile(f1);
         * t1.ArrangeGame(new SortedPlan(), 8); assertFalse(t1.SetGamePlan(-1, 1, 1,
         * 1)); assertFalse(t1.SetGamePlan(10, 1, 1, 1)); assertFalse(t1.SetGamePlan(1,
         * -1, 1, 1)); assertFalse(t1.SetGamePlan(1, 10, 1, 1));
         * assertFalse(t1.SetGamePlan(1, 1, -1, 1)); assertFalse(t1.SetGamePlan(1, 1,
         * 10, 1)); assertFalse(t1.SetGamePlan(1, 1, 1, -1));
         * assertFalse(t1.SetGamePlan(1, 1, 1, 10));
         * 
         * assertFalse(t1.SetGamePlan(1, 1, 2, 1)); assertFalse(t1.SetGamePlan(2, 1, 1,
         * 1));
         * 
         * t1.SetGamePlan(1, 1, 1, 2); List<List<Athlete>> gameplan=t1.getGamePlan();
