...
public class TrackGameTest {
	/*Testing stragety:	
	 *	1.setGamePlan:		1）group1，group2：超过已有组数，未超过已有组数，小于0
	 *						2）tracknum1，tracknum2：是合法轨道数，不是合法轨道数，小于0
	 *						3）group1>group2，group1<group2,group1==group2
	 *						4）tracknum1>tracknum2,tracknum1<tracknum2,tracknum1==tracknum2
	 *						5）交换的两个运动员是否为同一个
	 *	2.islegalOneGroup	1）c:中心物体是否是null
	 *						2）c中轨道数是否为非法数字（是否在[4,10]之间）
	 *						3）c中每个跑道上的运动员数量是否是合法的：每条跑道上只能有一名运动员
	 *	3.getAthleteNum		1）tracknum是否合法，是否在[4,10]区间内
	 *						2）输入中是否存在不包含任何物体的轨道
	 *	4.noSameAthInBothOrbits	1）两轨道上是否有相同的物体：
	 *							2）两轨道上是否存在其中没有物体的情况
	 */
	@Test
	public void testInitWithFile() throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
		...
		c1.addPhyToTrack(a1, 1, 1);c1.addPhyToTrack(a2,10, 2);
		...
		c1.addPhyToTrack(a1, 1, 1);c2.addPhyToTrack(a2, 1, 2);
		...
		c1=CircularOrbit.empty();c2=CircularOrbit.empty();
		for(int i=1;i<=5;i++) {
			c1.addInsideTrack(i);c2.addInsideTrack(i);
		}
		...
		c1.addPhyToTrack(a1, 1, 1);c1.addPhyToTrack(a2, 1, 2);c1.addPhyToTrack(a3, 1, 3);
		c1.addPhyToTrack(a4, 1, 4);c2.addPhyToTrack(a6, 1, 1);
		assertFalse(t1.isLegal(clist));
		c1.addPhyToTrack(a5, 1, 5);
		assertTrue(t1.isLegal(clist));
		
		c2.addPhyToTrack(a7, 1, 2);c2.addPhyToTrack(a8, 1, 3);c2.addPhyToTrack(a9, 1, 4);
		c2.addPhyToTrack(a10, 1, 5);
		assertTrue(t1.isLegal(clist));
		c2.removePhy(a6, 1);c2.addPhyToTrack(a1, 1, 1);
		assertFalse(t1.isLegal(clist));
		
		/*此处由于assert测不到
		 * c2.removePhy(a1, 1);c2.addPhyToTrack(a6, 1, 1);c2.addPhyToTrack(a11, 10, 1);
		assertFalse(t1.isLegal(clist));
		 */
		...
		/*
		 * t1.ArrangeGame(new SortedPlan(), 5);
		List<List<Athlete>> gameplan=t1.getGamePlan();
		List<Athlete> gp=gameplan.get(0);
		assertEquals(1.0,gp.get(0).getBestScore(),0.001);
		...
		assertEquals(0.9,gp.get(7).getBestScore(),0.001);
		 ...
		 gp=gameplan.get(1);
		 assertNull(gp.get(0));assertNull(gp.get(1));assertNull(gp.get(2));
	...
	@Test
	public void testSetAndGetGamePlan() throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
		/*
		 * ApplicationFactory tf=new TrackGameFactory();
		TrackGame t1=(TrackGame)tf.getApplication();
		File f1=new File("src\\inputfile\\TrackGameForTest.txt");
		t1.initFromFile(f1);
		t1.ArrangeGame(new SortedPlan(), 8);
		assertFalse(t1.SetGamePlan(-1, 1, 1, 1));
		assertFalse(t1.SetGamePlan(10, 1, 1, 1));
		assertFalse(t1.SetGamePlan(1, -1, 1, 1));
		assertFalse(t1.SetGamePlan(1, 10, 1, 1));
		assertFalse(t1.SetGamePlan(1, 1, -1, 1));
		assertFalse(t1.SetGamePlan(1, 1, 10, 1));
		assertFalse(t1.SetGamePlan(1, 1, 1, -1));
		assertFalse(t1.SetGamePlan(1, 1, 1, 10));

		assertFalse(t1.SetGamePlan(1, 1, 2, 1));
		assertFalse(t1.SetGamePlan(2, 1, 1, 1));

		t1.SetGamePlan(1, 1, 1, 2);
		List<List<Athlete>> gameplan=t1.getGamePlan();
		List<Athlete> gp=gameplan.get(0);
		assertEquals(1.0,gp.get(1).getBestScore(),0.001);
		assertEquals(0.8,gp.get(0).getBestScore(),0.001);
		
		t1.SetGamePlan(1, 1, 2, 4);
		gameplan=t1.getGamePlan();
		gp=gameplan.get(0);
		assertEquals(0.1,gp.get(0).getBestScore(),0.001);
		gp=gameplan.get(1);
