 ...
 * 
 * <p>
 * PS2 instructions: you MUST NOT add constructors, fields, or non-@Test methods to this class, or
 * change the spec of {@link #emptyInstance()}. Your tests MUST only obtain Graph instances by
 * calling emptyInstance(). Your tests MUST NOT refer to specific concrete implementations.
 */
public abstract class GraphInstanceTest {
   ...
   * 
   * set(): weight输入值：0，非0 source到target的边：已存在，不存在 顶点source在图中：已存在，不存在 顶点target在图中：已存在，不存在
   * 
   * remove(): 输入值vertex:为空，在原图中已存在，原图中不存在 是否有邻接vertex的边：无，有以vertex为起点的边，有以vertex为终点的边
   * 
  ...
  public void testInitialVerticesEmpty() {
    ...
    assertEquals("expected new graph to have no vertices", Collections.emptySet(),
        emptyInstance().vertices());
  }
