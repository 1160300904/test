...
public class TestAppHandler {
	/*Testing strategy:
	 * 	1.FilesyntaxHandling			1）输入的字符串中有哪类信息：App/User/Period/InstallLog/UninstallLog
	 * 											UsageLog/Relation
	 * 	2.handleRelation				1）输入的错误类型：信息没有被<,>括起来/仅有一个或没有app/任意一个app的名称
	 * 														为非法的
	 * 	3.handleUseLog					1）输入的错误类型：信息没有被<,>括起来/句中信息缺失/一个app的名称
	 * 														为非法的/时间的格式不对/句中的时长不是一个正数
	 * 	4.handleUnInstallLog			1）输入的错误类型：信息没有被<,>括起来/句中信息缺失/一个app的名称
	 * 														为非法的/时间的格式不对/句中的时长不是一个正数
	 * 	5.handleInstallLog				1）输入的错误类型：信息没有被<,>括起来/句中信息缺失/一个app的名称
	 * 														为非法的/时间的格式不对/句中的时长不是一个正数
	 * 	6.handleApp						1）输入的错误类型：信息没有被<,>括起来/句中信息缺失/一个app的名称
	 * 														为非法的/app公司信息非法/app版本信息非法/app功能信息非法
	 * 														app商业领域信息非法
	 * 	7.handleUser					1）输入的错误类型：信息没有'-'符号/句中无用户名/用户名中有空格/用户名中有其他非法字符
	 * 	8.handlePeriod					1）输入的错误类型：信息没有'-'符号/句中无分割信息
	 * 										/period中不是Hour|Day|Week|Month中的一个
	 * 														
	 */		
	@Test
