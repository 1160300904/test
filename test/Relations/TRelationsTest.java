...
public class TRelationsTest {
	/*Testing strategy:
	 * 	1.two constructors:		input :null, not null.
	 * 	2.addRelation(): 		relation has existed in current relations, or not.
	 * 							the object on the relation has existed in current relations, or not.
	 * 	3.deleRelation(): 		relation has existed in current relations, or not.
	 * 							the object on the relation has existed in current relations, or not.
	 * 	4.relatedTo():			the relation between the inputs has existed in current relations, or not.
	 * 	5.make sure this relation class works depends on the content of objects, not on its reference.
	 * 	6.Test on some reall PhysicalObjects.
	 */
	@Test
	public void testTRlations() {
		...
		tra.addRelation(a11, a2);tra.addRelation(a2, a11);
		tra.addRelation(a11, a3);tra.addRelation(a3, a11);
		tra.addRelation(a3, a2);tra.addRelation(a2, a3);tra.addRelation(a2, a3);
		assertTrue(tra.relatedTo(a11, a2));assertTrue(tra.relatedTo(a3, a2));
		assertTrue(tra.relatedTo(a11, a3));assertFalse(tra.relatedTo(a11, a4));
		assertTrue(tra.relatedTo(a12, a3));assertFalse(tra.relatedTo(a12, a4));
		...
		tr1.addRelation("1", "2");tr1.deleRelation("1", "2");
		/*
		 * Graph<PersonalApp> g=Graph.empty();
		g.set(wechat, QQ, 1);g.set(wechat, Eleme, 1);g.set(QQ, weibo, 1);g.set(Didi, BaiduMap, 1);
