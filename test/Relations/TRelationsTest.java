...
public class TRelationsTest {
   ...
   * existed in current relations, or not. the object on the relation has existed in current
   * relations, or not. 3.deleRelation(): relation has existed in current relations, or not. the
   * object on the relation has existed in current relations, or not. 4.relatedTo(): the relation
   * between the inputs has existed in current relations, or not. 5.make sure this relation class
   * works depends on the content of objects, not on its reference. 6.Test on some reall
   * PhysicalObjects.
   */
  @Test
  public void testTRlations() {
    ...
    /*
     * Graph<PersonalApp> g=Graph.empty(); g.set(wechat, QQ, 1);g.set(wechat, Eleme, 1);g.set(QQ,
     * weibo, 1);g.set(Didi, BaiduMap, 1); System.out.println(g.targets(wechat));
     */
  }
}
