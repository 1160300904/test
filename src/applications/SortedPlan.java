...
public class SortedPlan implements GamePlan {
	...
	public List<List<Athlete>> plan(List<Athlete> athletes, int tracknum) {
		...
		List<List<Athlete>> retlist=new ArrayList<>();List<Athlete> grouplist;
		
		int leftathnum=athletes.size();int i=0;
		while(leftathnum>=tracknum) {
			...
			i+=tracknum;leftathnum-=tracknum;
		}
		if(leftathnum>0) {
			grouplist=new ArrayList<>();
