...
public class TrackGame extends ConcreteCircularOrbit<String, Athlete> {
    ...
    /*
     * Abstract Function: 1.athletes represents the athletes generated from the file
     * 2.gameplan represents the game plan generated from function ArrangeGame().
     * 1)Each inside list of athletes is one match, so the whole gameplan is the
     * matches generated in some particular ways. 2)In each inside list, each index
     * from 0 to its size, represents a track, on which is an athlete. So, using
     * this inside list, we can generate a match, the index of athlete maps to the
     * track number he owns. 3.game represents the type of the game. 4.numoftracks
     * represents the track numbers read from the file.
     */

    /*
     * Rep Invariant: 1.Any two athletes in the 'athletes' fields must not be the
     * same. 2.No matter how game plan change, the track number of each match must
     * equals the 'numoftracks' 3.No matter how game plan change, if the last match
     * is not full, all other matches must be full 4.game must be 100, or 200, or
     * 400, no other number is premitted. 4.numoftracks must in a range of [4,10].
     */
    void checkRep() {
     ...
     * @throws FileNotFoundException    if the initial file is not found in the
     *                                  disk.
     * @throws FileSyntaxException      if the syntax error of the file is avoided.
     * @throws RepeatedObjectsException if the athlete repeated in the file you
     *                                  provide.
     */
    public boolean initFromFile(File file) throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
        assert file != null;
        ...
        while (input.hasNext()) {
            ...
            if (str.matches("Game\\s*::=\\s*(100|200|400)")) {
                ...
                LogRecord lr = new LogRecord(Level.INFO, "Exception" + ",FileSyntaxException," + mes + ",try again");
                this.log.log(lr);
                this.logsaver.add(lr);
     ...
     * 
     * @param gp       the gameplan you provide to arrange an track game. See in
     *                 class GamePlan. it cannot be null.
     * @param tracknum the track number of the game you want to arrange. it must in
     *                 the range of [4,10].
    ...
    public boolean ArrangeGame(GamePlan gp, int tracknum) {
        ...
        /*
         * if(input.equals("random")) { gp=new RandomPlan(); }else if
         * (input.equals("sorted")){ gp=new SortedPlan(); }else { return false; }
         */
    ...
    /**
     * Set the game plan according the input.The athlete in group1, track1, and the
     * athlete in group2 ,track2, are exchanged. You can rearrange the plan with
     * tracks with no one on it.
     * 
    ...
    /**
     * Get the game plan generated by the function ArrangeGame().You cannot modify
     * this list.
     * 
     ...
     * 
     * @param athset the set you want to get athlete from. The set must contains
     *               only one athlete.
     ...
     * @return true if there is no two same athletes in two circular orbit. False
     *         otherwise.
     */
    public boolean noSameAthInBothOrbits(CircularOrbit<String, Athlete> orbit1, CircularOrbit<String, Athlete> orbit2,
            int tracknum) {
        Athlete a1 = null;
        Athlete a2 = null;
    ...
    public boolean isLegal(List<CircularOrbit<String, Athlete>> clist) {
        ...
        for (int i = 0; i < listsize; i++) {
            for (int j = 0; j < listsize; j++) {
                if ((i != j) && noSameAthInBothOrbits(clist.get(i), clist.get(j), tracknum) == false) {
                    return false;
                }
            }
            if (isLegalOneGroup(clist.get(i)) == false)
                return false;
        }
        return true;
    }
     ...
     */
    public boolean isLegalForTwoOrbits(CircularOrbit<String, Athlete> c1, CircularOrbit<String, Athlete> c2) {
