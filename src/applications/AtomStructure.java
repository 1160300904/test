...
public class AtomStructure extends ConcreteCircularOrbit<Nucleus, Electron> {
    ...
    /*
     * Abstract function: 1.elementname is the name of the element. 2.numoftracks is
     * the track number read from the file. 3.elefromfile contains the information
     * of electron on tracks around the nucleus.
     */
    /*
     * Rep Invariant 1.numoftracks must be a positive number. 2.each element in
     * elefromfile must be a positive number. 3.the track number of this atom
     * structure must equals with the max number in the elefromfile. And each track
     * below that track must be declared in elefromfile.
     */
    void checkRep() {
     ...
     * @throws FileSyntaxException       If the sentences in the file has syntax
     *                                   errors.
     * @throws FileInfoConflictException If the information in the file conflicts
     *                                   with each other.
     */
    public void initWithFile(File file) throws FileNotFoundException, FileSyntaxException, FileInfoConflictException {
        assert file != null;
        ...
        if (trackNumConflict() == false) {
            String mes = "Self conflict on number of tracks of the atom: " + "From file is " + this.numoftracks
                    + " from track info is: " + this.elefromfile.keySet().size();
            LogRecord lr = new LogRecord(Level.INFO, "Exception" + ",FileInfoConflictException," + mes + ",try again");
            this.log.log(lr);
            this.logsaver.add(lr);
    ...
    /**
     * get an unmodifiable view of the elements on track, which can be used to
