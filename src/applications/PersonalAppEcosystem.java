...
public class PersonalAppEcosystem extends ConcreteCircularOrbit<User, PersonalApp> {
    ...
    /*
     * Abstract Function: 1.inlog represent the install log read from the file.
     * 2.uninlog represent the uninstall log initialized from the file. 3.uselog
     * represent the use log initialized from the file. 4.durations maps each
     * useagelog to its useing duration. 5.relations represent the relations between
     * two apps initialized from the file. 6.splitby represents what strategy will
     * be used to split the time line of this phone. 7.username represents the user
     * name of this ecosystem. 8.apps is a map, which maps names of each app to
     * itself. 9.phone is simulator of a phone, we use it to simulate the install,
     * uninstall, use operation to arrange the log from the file, so that we can
     * find out the details of each period of the phone. 10.ecos has the infomation
     * to generate a series of app ecosystem, which is generated by the function
     * ArrangeEcos() 11.timenode is a list of time node represents the split point
     * in time of the series of app ecosystems according to the file.
     */
    /*
     * Rep Invariant: 1.Null cannot exists in the inlog list. 2.Null cannot exists
     * in the uninlog list. 3.Null cannot exists in the uselog list. each element in
     * this list, must exists in the key set of duration map too. 4.the key and
     * value in durations cannot be null. each element in key set of duration map,
     * must exists in the uselog list too. the integer value of duration map must be
     * positive. 5.for all the elements in the relation map, it cannot be null all
     * the elements in the relation map must exists in the key set of HashMap apps
     * 6.splitby can only be Hour|Day|Week|Month 7.each elements in apps map cannot
     * be null. in map app, each string must map to a personalapp has a name of
     * itself.
     */
    ...
    void checkRep() {
        ...
        assert splitby.equals("Hour") || splitby.equals("Day") || splitby.equals("Week") || splitby.equals("Month");
    ...
    /**
     * initialize the app system from the file you provide. Then you can do other
     * thing this application provides.
     * 
     * @param file The file you offer to initiate.
     * @return true if the progress succeed.
     * @throws FileNotFoundException    If the file cannot be found, throws
     *                                  FileNotFoundException
     * @throws FileSyntaxException      If there are syntax errors in the file you
     *                                  provide, throws FileSyntaxException.
     * @throws RepeatedObjectsException If there are duplicated names in the file
     *                                  you provides, throws
    ...
    public boolean initFromFile(File file) throws FileNotFoundException, FileSyntaxException, RepeatedObjectsException {
        ...
        while (input.hasNext()) {
            ...
            if (str.matches(userpstr)) {
                ...
                PersonalApp pa = PersonalAppFactory.getInstance(appname, company, version, function, businessarea);
                 ...
                 * group(3)); System.out.println(matcher.group(4));
        ...
        checkRep();
        /*
         * debug System.out.println();System.out.println();System.out.println();
         * for(LogElement le:this.inlog) { System.out.println(le.toString()); }
         * System.out.println(); for(LogElement le:this.uninlog) {
         * System.out.println(le.toString()); } System.out.println(); for(LogElement
         * le:this.uselog) { System.out.println(le.toString()); } System.out.println();
         * for(Entry<LogElement,Integer> e:this.durations.entrySet()) {
         * System.out.println(e.getKey().getName()+" for "+e.getValue()); }
         * System.out.println(); for(String s:this.relations.keySet()) { HashSet<String>
         * set=this.relations.get(s); for(String s1:set) { System.out.println(s+" "+s1);
         * } } System.out.println(); System.out.println(this.splitby);
         */
        return true;
    }

    /**
     * Add relations between the two objects you provides. It doesn't refer to a
     * specific circular orbit, It means the relations in the global perspective.
     * 
     ...
     * @return true if these two objecds are related with each other, false
     *         otherwise.
     */
    ...
    /**
     * Get an unmodifiable map view of the relations between objects in the personal
     * app ecosystem.
     * 
     * @return an unmodifiable map view of the relations between objects in the
    ...
    /**
     * Given three indices, get the earliest MyDate by returning 1, or 2, or 3. 1
     ...
     * @return 1, or 2, or 3 which means the list sequence number which has the
     *         earliest MyDate. 1 means the first list,2 mean the second, 3 means
     *         the third.
     */
    ...
    /**
     * arrange the personal app ecosystem according to the informations read from
     * the file.
     * 
     * @throws FileInfoConflictException If the file information is conflict in
     *                                   itself, throw FileInfoConflictException.
     */
    public void ArrangeEcos() throws FileInfoConflictException {
         ...
         * for(LogElement le:this.uninlog) { System.out.println(le.toString());
         * }System.out.println();System.out.println();System.out.println();
         * for(LogElement le:this.uselog) { System.out.println(le.toString());
         * }System.out.println();System.out.println();System.out.println();
         */
        int inindex = 0;
        int uninindex = 0;
         ...
         * periodend=this.uninlog.get(0).getDate(); }else {
         * periodend=this.uselog.get(0).getDate(); }
         */
    ...
    /**
     * Get the informations of the ecosystem arranged by the method ArrangeEcos(),
     * it can be used to draw a sequence of circular orbit of app ecosystems.
     * 
     * @return
     */
    ...
    /**
     * Get the time stamp of each period of the app ecosystems. For each two time
     * stamps, one is the beginning of a period, the other is the ending of the
     * period.
     * 
    ...
    /**
     * clear up the phone and add a new element with the information of the phone
     * into the list of ecos.
     * 
     * @param fremap
     * @param periodend
     * @param timemap
     */
    private void clear(HashMap<String, Integer> fremap, MyDate periodend, HashMap<String, Integer> timemap) {
     ...
     * @return true if the progress succeed, false otherwise.
     * @throws FileInfoConflictException 1.the app didn't declared in the file.
     *                                   2.the app is installed more than once.
     */
     ...
     * @return true if the progress succeed, false otherwise.
     * @throws FileInfoConflictException 1.the app didn't declared in the file.
     *                                   2.the app is used before install.
     */
    private boolean useSimu(int useindex) throws FileInfoConflictException {
        ...
        /*
         * debug Integer i=durations.get(le); if(i==null) { System.out.println("null");
         * } int dur=i;
         */
     ...
     * @return true if the progress succeed, false otherwise.
     * @throws FileInfoConflictException 1.the app didn't declared in the file.
     *                                   2.the app is uninstalled more than once.
     */
    private boolean UninstallSimu(int uninindex, HashMap<String, Integer> fremap, HashMap<String, Integer> timemap)
    ...
    /**
     * Merge a single uselist into the fremap and timemap, which can be used in
     * arranging the ecosystem.
     * 
    ...
    /**
     * Test if two apps in the same orbit is legal according to the rules of the
     * system.
     * 
     ...
     * @return true if the progress succeed, false otherwise.
     */
    public boolean synthensisLegal(CircularOrbit<User, PersonalApp> c, String appname1, String appname2,
            int orbitlistindex) {
        ...
        for (int i = 1; i <= objlist.size(); i++) {
            ...
            /*
             * if(set1.contains(app1)) { app1track=i; } if(set1.contains(app2)) {
             * app2track=i; }
             */
        }
 ...
 *
 */
class LogElement implements Comparable<LogElement> {

    private String name;
    private MyDate date;

    /*
     * Abstract Function: 1.name is the name of the app in this log element. 2.date
     * is the time when this log event happens.
     */
    /*
     * Rep Invariant: none.
     */
