...
public class PersonalAppEcosystem extends ConcreteCircularOrbit<User, PersonalApp> {
	...
	/*Abstract Function:
	 *	1.inlog represent the install log read from the file.
	 *	2.uninlog represent the uninstall log initialized from the file.
	 *	3.uselog represent the use log initialized from the file.
	 *	4.durations maps each useagelog to its useing duration.
	 *	5.relations represent the relations between two apps initialized from the file.
	 * 	6.splitby represents what strategy will be used to split the time line of this phone.
	 * 	7.username represents the user name of this ecosystem.
	 * 	8.apps is a map, which maps names of each app to itself.
	 * 	9.phone is simulator of a phone, we use it to simulate the install, uninstall, use operation
	 * 		to arrange the log from the file, so that we can find out the details of each period of the 
	 * 		phone.
	 * 	10.ecos has the infomation to generate a series of app ecosystem, which is generated by the 
	 * 		function ArrangeEcos()
	 * 	11.timenode is a list of time node represents the split point in time
	 * 		of the series of app ecosystems according to the file.
	 */	
	/*Rep Invariant:
	 * 	1.Null cannot exists in the inlog list.
	 * 	2.Null cannot exists in the uninlog list.
	 * 	3.Null cannot exists in the uselog list. each element in this list, must exists in the 
	 * 		key set of duration map too.
	 * 	4.the key and value in durations cannot be null.
	 * 		each element in key set of duration map, must exists in the uselog list too.
	 * 		the integer value of duration map must be positive.
	 * 	5.for all the elements in the relation map, it cannot be null
	 * 		all the elements in the relation map must exists in the key set of HashMap apps
	 *  6.splitby can only be Hour|Day|Week|Month 
	 *  7.each elements in apps map cannot be null.
	 *  	in map app, each string must map to a personalapp has a name of itself.
	 */
	...
	void checkRep(){
		...
		Set<String> relationValueMap;Set<String> appKeySet=this.apps.keySet();
		for(String s1:this.relations.keySet()) {
			assert s1!=null;assert appKeySet.contains(s1);
	...
	/**
	 * initialize the app system from the file you provide. Then you can do other thing this application
	 * provides.
	 ...
	 * @throws FileSyntaxException		If there are syntax errors in the file you provide, 
	 * 									throws FileSyntaxException.
	 * @throws RepeatedObjectsException		If there are duplicated names in the file you provides,
	...
	public boolean initFromFile(File file) throws FileNotFoundException,FileSyntaxException,
		...
		while(input.hasNext()) {
			...
			if(str.matches(userpstr)) {
				matcher=userp.matcher(str);matcher.find();
				...
				PersonalApp pa=PersonalAppFactory.getInstance(appname, company, 
															version, function, businessarea);
				...
				matcher=intp.matcher(str);matcher.find();
				...
				matcher=unintp.matcher(str);matcher.find();
				int year=Integer.valueOf(matcher.group(1));
				int month=Integer.valueOf(matcher.group(2));
				int day=Integer.valueOf(matcher.group(3));
				int hour=Integer.valueOf(matcher.group(4));
				...
				matcher=usep.matcher(str);matcher.find();
				int year=Integer.valueOf(matcher.group(1));
				int month=Integer.valueOf(matcher.group(2));
				int day=Integer.valueOf(matcher.group(3));
				int hour=Integer.valueOf(matcher.group(4));
				...
				matcher=relp.matcher(str);matcher.find();
				...
				matcher=perp.matcher(str);matcher.find();
		...
		for(LogElement le:this.inlog) {
			System.out.println(le.toString());
		}
		System.out.println();
		for(LogElement le:this.uninlog) {
			System.out.println(le.toString());
		}
		System.out.println();
		for(LogElement le:this.uselog) {
			System.out.println(le.toString());
		}
		System.out.println();
		...
		System.out.println();
		for(String s:this.relations.keySet()) {
			HashSet<String> set=this.relations.get(s);
			for(String s1:set) {
	...
	/**
	 * Add relations between the two objects you provides. It doesn't refer to a specific circular orbit,
	 * It means the relations in the global perspective.
	...
	public boolean isRelated(String s1,String s2) {
		HashSet<String> relset=this.relations.get(s1);
		if(relset==null)	return false;
		return relset.contains(s2);
	}
	/**
	 * Get an unmodifiable map view of the relations between objects in the personal app ecosystem.
	 * @return	an unmodifiable map view of the relations between objects in the personal app ecosystem.
	 */
	public Map<String,HashSet<String>> getRelationMap(){
		return Collections.unmodifiableMap(this.relations);
	}
	/**
	 * Given three indices, get the earliest MyDate by returning 1, or 2, or 3. 1 means the first list,
	 * 2 mean the second, 3 means the third.
	 ...
	 * @return		1, or 2, or 3 which means the list sequence number which has the earliest MyDate.
	 * 				 1 means the first list,2 mean the second, 3 means the third.
	 */
	 ...
	 * 			If the file information is conflict in itself, throw FileInfoConflictException.
	 */
	public void ArrangeEcos() throws FileInfoConflictException {
		Collections.sort(this.inlog);Collections.sort(this.uninlog);Collections.sort(this.uselog);
		/*
		 * for(LogElement le:this.inlog) {
			System.out.println(le.toString());
		}System.out.println();System.out.println();System.out.println();
		 ...
		 */
		int inindex=0;int uninindex=0;int useindex=0;
		int insize=this.inlog.size();int uninsize=this.uninlog.size();int usesize=this.uselog.size();
		//System.out.println(insize+" "+uninsize+" "+usesize);
		int min;
		HashMap<String,Integer> fremap=new HashMap<>();HashMap<String,Integer> timemap=new HashMap<>();
		//1.find the min time for the total log.
		min=getMinMyDate(inindex,uninindex,useindex,insize,uninsize,usesize);
		MyDate periodend;MyDate handdate;
		/*
		 * if(min==1) {
			periodend=this.inlog.get(0).getDate();
		}else if(min==2) {
			periodend=this.uninlog.get(0).getDate();
		}else {
		...
		while(inindex<insize||uninindex<uninsize||useindex<usesize) {
			...
			if(handdate.compareTo(periodend)>0) {
				...
				fremap=new HashMap<>();timemap=new HashMap<>();
				this.timenode.add(periodend);
				periodend=MyDate.addBaseOnSplit(splitby, periodend);
			}
	...
	/**
	 * clear up the phone and add a new element with the information of the 
	 * phone into the list of ecos.
	 * @param fremap
	 * @param periodend
	 * @param timemap
	 */
	private void clear(HashMap<String,Integer> fremap,
									MyDate periodend, HashMap<String,Integer> timemap) {
		List<Integer> uselist;List<AppUseNote> usenoteoneperiod;
	...
	private boolean useSimu(int useindex) throws FileInfoConflictException {
		LogElement le=this.uselog.get(useindex);String name=le.getName();
		...
		if((uselist=phone.get(name))==null) {
			String mes=name+" is not installed in the phone currently.";
			LogRecord lr=new LogRecord(Level.INFO, "Exception"+",FileInfoConflictException,"+
			mes+",try again");
			this.log.log(lr);
			this.logsaver.add(lr);
	...
	/**
	 * Merge a single uselist into the fremap and timemap, which can be used in arranging the ecosystem.
	...
	private boolean MergeSingle(String name,List<Integer> uselist,HashMap<String,Integer> fremap,
		...
		int unmergefreq=0;int unmergetime=0;
	...
	public boolean synthensisLegal(CircularOrbit<User, PersonalApp> c,String appname1,
		...
		AppUseNote note1=null;AppUseNote note2=null;
		...
		int app1track=0;int app2track=0;
		PersonalApp app1=this.apps.get(appname1);PersonalApp app2=this.apps.get(appname2);
		for(int i=1;i<=objlist.size();i++) {
			...
			/*
			 * if(set1.contains(app1)) {
				app1track=i;
			}
			if(set1.contains(app2)) {
				app2track=i;
			}
			 */
		}
		int syn1=note1.getSynthesize();int syn2=note2.getSynthesize();
		if(app1track>app2track) {
			if(syn2>syn1)	return true;
		}
		if(app1track<app2track) {
			if(syn2<syn1)	return true;
		}
...
class LogElement implements Comparable<LogElement>{
	...
	private MyDate date;
	/*Abstract Function:
	 * 	1.name is the name of the app in this log element.
	 * 	2.date is the time when this log event happens.
	 */
	/*Rep Invariant:
	 * 	none.
	 */
