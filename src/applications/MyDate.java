...
/**
 * This is an immutable type represents the date in the PersonalAppEcosystem.
 * It is IMMUTABLE.
 */
public class MyDate implements Comparable<MyDate> {
	...
	/*Abstract Function:
	 * 	1.t means the time stamp of this MyDate class.
	 *  2.t is immutable type so it is safer to use it as a field.
	 */
	/*Rep Invariant:
	 * 	none.
	 */
	...
	MyDate(int year,int month,int day,int hour,int minute,int second){
		assert year>=0;assert month>=0;assert day>=0;
		assert hour>=0;assert minute>=0;assert second>=0;
	...
	/*
	 * MyDate truncYear() {
		LocalDateTime l=t.truncatedTo(ChronoUnit.YEARS);
		return new MyDate(l);
	}
	 */
	/**
	 * truncate the MyDate instance by month.
	 * @return		A new Instance which all its fields are zero except for year 
	 * 				and month based on its old time.
	 */
	/*
	 * MyDate truncMonth() {
		LocalDateTime l=t.truncatedTo(ChronoUnit.MONTHS);
		return new MyDate(l);
	}
	 */
	/**
	 ...
	 * @return		A new Instance which all its fields are zero except for year, 
	 * 				month and day based on its old time.
	 */
	MyDate truncDay() {
	 ...
	 * @return		A new Instance which all its fields are zero except for year, 
	 * 				month and day and hour based on its old time.
	 */
	MyDate truncHour() {
	 ...
	 * @return		A new Instance which all its fields are zero except for year, 
	 * 				month and day and hour and minute based on its old time.
	 */
	MyDate truncMinute() {
	 ...
	 * @return		A new Instance which all its fields are zero except for year, 
	 * 				month and day and hour and minute and second based on its old time.
	 */
	MyDate truncSecond() {
	...
	public static int getMin(MyDate d1,MyDate d2,MyDate d3) {
		assert d1!=null;assert d2!=null;assert d3!=null;
		MyDate min=null;int retvalue;
