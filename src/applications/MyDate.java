...
/**
 * This is an immutable type represents the date in the PersonalAppEcosystem. It is IMMUTABLE.
 */
public class MyDate implements Comparable<MyDate> {
  ...
  /*
   * Abstract Function: 1.t means the time stamp of this MyDate class. 2.t is immutable type so it
   * is safer to use it as a field.
   */
  /*
   * Rep Invariant: none.
   */
  ...
  MyDate addYear(int y) {
    assert y >= 0;
    LocalDateTime l = time.plusYears(y);
  ...
  MyDate addMonth(int m) {
    assert m >= 0;
    LocalDateTime l = time.plusMonths(m);
    // defensive copy here to avoid rep exposure.
    return new MyDate(l);
  }
  ...
  MyDate addDay(int d) {
    assert d >= 0;
    LocalDateTime l = time.plusDays(d);
    // defensive copy here to avoid rep exposure.
    return new MyDate(l);
  }
  ...
  MyDate addHour(int y) {
    assert y >= 0;
    LocalDateTime l = time.plusHours(y);
    return new MyDate(l);
  }
  ...
  MyDate addMinute(int y) {
    assert y >= 0;
    LocalDateTime l = time.plusMinutes(y);
    return new MyDate(l);
  }
  ...
  MyDate addSecond(int y) {
    assert y >= 0;
    LocalDateTime l = time.plusSeconds(y);
    return new MyDate(l);
  }
  /**
   * truncate the MyDate instance by year.
   * 
   * @return A new Instance which all its fields are zero except for year based on its old time.
   */
  /*
   * MyDate truncYear() { LocalDateTime l=t.truncatedTo(ChronoUnit.YEARS); return new MyDate(l); }
   */
  /**
   * truncate the MyDate instance by month.
   * 
   * @return A new Instance which all its fields are zero except for year and month based on its old
   *         time.
   */

  /*
   * MyDate truncMonth() { LocalDateTime l=t.truncatedTo(ChronoUnit.MONTHS); return new MyDate(l); }
   */
  /**
   * truncate the MyDate instance by day.
   * 
   * @return A new Instance which all its fields are zero except for year, month and day based on
   *         its old time.
   */
  MyDate truncDay() {
    LocalDateTime l = time.truncatedTo(ChronoUnit.DAYS);
    return new MyDate(l);
  }
   ...
   * 
   * @return A new Instance which all its fields are zero except for year, month and day and hour
   *         based on its old time.
   */
  MyDate truncHour() {
    LocalDateTime l = time.truncatedTo(ChronoUnit.HOURS);
    return new MyDate(l);
  }
   ...
   * 
   * @return A new Instance which all its fields are zero except for year, month and day and hour
   *         and minute based on its old time.
   */
  MyDate truncMinute() {
    LocalDateTime l = time.truncatedTo(ChronoUnit.MINUTES);
    return new MyDate(l);
  }
   ...
   * 
   * @return A new Instance which all its fields are zero except for year, month and day and hour
   *         and minute and second based on its old time.
   */
  MyDate truncSecond() {
    LocalDateTime l = time.truncatedTo(ChronoUnit.SECONDS);
    return new MyDate(l);
  }
  ...
  public int compareTo(MyDate d) {
    return time.compareTo(d.getTimeStamp());
  }

  /**
   * Given three MyDate instance, return serial number of MyDate which has the earliest time.
   * 
   ...
   * @return 1 means d1 is the earliest, 2 means d2 is the earliest, 3 means d3 is the earliest.
   */
