...
public class CircularOrbitAPIs<L, E> {
    ...
    /**
     * Get the entropy of the input CircularOrbit system, basing on the distribution
     * of the objects on tracks.
     * 
    ...
    /**
     * Get the logical distance between two objects, which is the minimum number of
    ...
    /**
     * calculate the difference between two circular orbits. The defination of
     * Difference is in the Difference API.
     * 
     * @param c1 one of the circular orbit you want to calculate with.
     * @param c2 another circular orbit you want to calculate with.
     * @return the difference between two circular orbits. The definition of
     *         Difference is in the Difference API.
     */
    public Difference getDifference(CircularOrbit<L, E> c1, CircularOrbit<L, E> c2) {
        ...
        if (cent instanceof Nucleus) {
            return getAtomDifference((CircularOrbit<Nucleus, Electron>) c1, (CircularOrbit<Nucleus, Electron>) c2);
        }
     ...
     *         Difference is in the Difference API.
     */
    public Difference getAtomDifference(CircularOrbit<Nucleus, Electron> c1, CircularOrbit<Nucleus, Electron> c2) {
    ...
    /**
     * Get the difference between two circular orbit represents two
    ...
    private String bothHaveApps(HashSet<PersonalApp> s1, HashSet<PersonalApp> s2) {
        ...
        Iterator<PersonalApp> it = rets1.iterator();
        PersonalApp pa = null;
        while (it.hasNext()) {
            pa = it.next();
            if (s2.contains(pa)) {
     ...
     *         Difference is in the Difference API.
     */
    public Difference getRaceGameDifference(CircularOrbit<String, Athlete> c1, CircularOrbit<String, Athlete> c2) {
        Difference d = Difference.getInstance();
        int c1tracknum = c1.TrackAmount();
        int c2tracknum = c2.TrackAmount();
        if (c1tracknum != c2tracknum) {
    ...
    private String getTrackDiffInRaceGame(HashSet<Athlete> c1ath, HashSet<Athlete> c2ath) {
        int c1size = c1ath.size();
        int c2size = c2ath.size();
