...
public class ConcreteVerticesGraph<L> implements Graph<L> {
	 ...
	 *	each element in the 'vertices' list represents a source vertex and its outward edges
     */
    /*Representation invariant:
     * 	each element in vertices shouldn't be null.
     * 	the 'source' in each elements in vertices should be different.
     */
    /*Safety from rep exposure:
     *  1. all fields are private.
     *  2. all parameters in all functions except vertices() are of immutable type.
     *  3. as for vertices(), it returns a new HashSet to client.
     *  4. as for source() and targets(), they returns a new HashMap to client.
     *  5. other functions returns immutable types, such as int or boolean.
     */
    ...
    @Override public int set(L source, L target, int weight) {
    	Vertex<L> existedv=null;TarAndWei<L> existedt=null;int oldweight;
    	for(Vertex<L> v:vertices) {
        ...
        if(weight!=0) {
        	...
        	if(existedv!=null&&existedt==null) {
        		existedv.getTargets().add(new TarAndWei<L>(weight,target));
        		vertices.add(new Vertex<L>(target));
        		return 0;
        	}else {
        		vertices.add(new Vertex<L>(source,new TarAndWei<L>(weight,target)));
    ...
    @Override public boolean remove(L vertex) {
        Vertex<L> ver=null;Iterator<TarAndWei<L>> tawiter=null;
        for(Vertex<L> v:vertices) {
 ...
 * a Vertex instance represents a vertex of a graph and all of the outward-edges of the vertex.
 * 
...
class Vertex<L> {
    ...
    /*Abstraction function:
     * 	source represents a vertex of the graph.
     * 	targets represents the list of the edges which are from vertex source.
     */
    /*Representation invariant:
     * 	source and each one in targets shouldn't be null.
     * 	none of each two vertices are the same in targets and source.
     */
    /*Safety from rep exposure:
     * 	1. all fields are private.
     * 	2. since TarAndWei is a mutable type, then:
     * 	   1) in constructor Vertex(L source,TarAndWei<L> targetAndWeight), use defensive copy to make a
     * 		  copy of the targetAndWeight argument.
     * 	   2) in function getTargets(), returns an unmodifiable view of the list of target
     * 	   3) in function boolean addTarAndWei(TarAndWei<L> taw), use defensive copy to make a
     * 		  copy of the taw argument.
     * 	   4) in function getIterator(), it return a iterator wrapping a new list instance other than the 
     * 		  instance that targets points at.
     */
    ...
    /**
     * constructor of class Vertex. create a new Vertex with the starting point, and a TarAndWei instance
     * of a graph.
     * @param source			the starting point of the edge
     * @param targetAndWeight	a TarAndWei instance, which represents the weight and the target of an edge.
     */
 ...
 * It nearly represent a whole edge except the starting point.
 * It can be used in class Vertex.
 * It is mutable.
 *
...
class TarAndWei<L>{
	 ...
	 * 	both rep is immutable type. so there won't have rep exposure.
	 */
	private int weight;
	private L target;
