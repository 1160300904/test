...
public class AtomStructurePane extends CircularOrbitPane<Nucleus, Electron> {
  ...
  /*
   * Abstract function: 1.atomstruc is the instance of AtomStructure, which will provide
   * informations for the pane to draw. 2.transition is a button to transit the electrons on tracks
   * 3.filechooser and stage is used for user to choose the initial file. 4.atomhandler is the
   * exception handler of atom structure.
   */
  /*
   * Rep Invariant: none.
   */
  ...
  AtomStructurePane() {
    ...
    this.initBut.setOnAction(e -> {
      /*
       * File file=filechooser.showOpenDialog(stage); try { this.atomstruc.initWithFile(file); }
    ...
    setOrbit.setOnAction(e -> {
      ...
      LogRecord lr =
          new LogRecord(Level.INFO, "Operation" + ",SetOrbit," + "set orbit" + ",succeed");
      this.log.log(lr);
      this.logsaver.add(lr);
    });
    ...
    this.transition.setOnAction(e -> {
      ...
      s.close();
      LogRecord lr =
          new LogRecord(Level.INFO, "Operation" + ",Transition," + "transition" + ",succeed");
      this.log.log(lr);
      this.logsaver.add(lr);
    });
    /*
