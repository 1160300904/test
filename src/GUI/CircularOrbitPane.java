...
public class CircularOrbitPane<L, E> extends BorderPane {
  ...
  /*
   * Abstract function: 1.circlepane,outputpane,regularuse,specialuse,inputpane,rightpane are panes
   * of the orbit pane on GUI. 2.inputfield,outputfield are text fields which can deal with
   * informations. 3.drawBut,initBut,addTrackBut,addObjBut,remTrackBut,remObjBut,entroBut,
   * setOrbit,outputmes are buttons for users to make use of the system. 4.orbit is the circular
   * orbit the pane is going to draw.
   */
  /*
   * Rep Invariant: none.
   */
  ...
  CircularOrbitPane() {
    ...
    remTrackBut.setOnAction(e -> {
      ...
      LogRecord lr =
          new LogRecord(Level.INFO, "Operation" + ",DeleteTrack," + inputtext + ",succeed");
      this.log.log(lr);
      this.logsaver.add(lr);
    });
