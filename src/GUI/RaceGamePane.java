...
/**
 * This is the class of pane of TrackGame. The circular orbit of TrackGame
...
public class RaceGamePane extends CircularOrbitPane<String,Athlete> {
	...
	/*Abstract function:
	 * 	1.trackgame is used for generating the orbits of track game.
	 * 	2.arangeBut,setPlanBut,difBut,isLegalBut are buttons for users to use to operate the GUI.
	 *  3.filechooser,stage is used for the user to choose initial file of this system.
	 * 	4.trackHandler is an exception handler handle the exceptions in this system.
	 * 	11.orbitlist contains the orbits generated from TrackGame.
	 */
	/*Rep Invariant:
	 * 	none.
	 */
	...
	RaceGamePane() {
		...
		this.remObjBut.setOnAction(e->{
			...
			Map<Athlete, Double> athletesmap=this.orbit.getThetas();Athlete deleath=null;
		...
		arangeBut.setOnAction(e->{
			String inputtext=this.inputfield.getText();
			Scanner s=new Scanner(inputtext);
			String category= s.next();int tracknum=s.nextInt();
			GamePlan gp=null;
			...
			s.close();
			LogRecord lr=new LogRecord(Level.INFO, "Operation"+",ArrangeGame,"+
					inputtext+",succeed");
			this.log.log(lr);
			this.logsaver.add(lr);
		});
		setPlanBut.setOnAction(e->{
			String inputtext=this.inputfield.getText();
			Scanner s=new Scanner(inputtext);
			int group1=s.nextInt();int track1=s.nextInt();int group2=s.nextInt();int track2=s.nextInt();
			//System.out.println(group1+" "+track1+" "+group2+" "+track2);
			this.trackgame.SetGamePlan(group1, track1, group2, track2);
			this.FromPlanToOrbit();
			s.close();
			LogRecord lr=new LogRecord(Level.INFO, "Operation"+",SetPlan,"+
					inputtext+",succeed");
			this.log.log(lr);
			this.logsaver.add(lr);
		});
	...
	private void FromPlanToOrbit() {
		...
		int i=0;CircularOrbit<String,Athlete> cor=null;
