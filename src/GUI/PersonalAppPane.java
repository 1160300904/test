...
public class PersonalAppPane extends CircularOrbitPane<User, PersonalApp> {
   ...
   * 4.tracknum is the number of tracks on each app ecosystems. 5.curorbindex is the current index
   * of orbit in the orbit list. 6.pereco is used for generating the orbits of personal app
   * ecosystem. 7.namecirclemap maps each name of a app to its circle on pane.
   * 8.difBut,logdisBut,phyDisBut,isLegalBut are buttons for users to use to operate the GUI.
   * 9.filechooser,stage is used for the user to choose initial file of this system. 10.appHandler
   * is an exception handler handle the exceptions in this system. 11.orbitlist contains the orbits
   * generated from personal app ecosystem.
   */
  /*
   * Rep Invariant: none.
   */
  ...
  PersonalAppPane() {
    ...
    phyDisBut.setOnAction(e -> {
      ...
      this.outputfield.setText("The physical distance between " + app1 + " and " + app2 + " is "
          + String.valueOf(distance));
      s.close();
      LogRecord lr =
          new LogRecord(Level.INFO, "Operation" + ",PhysicalDistance," + inputtext + ",succeed");
      this.log.log(lr);
      this.logsaver.add(lr);
    });
    // initbut
    this.initBut.setOnAction(e -> {
      /*
       * File file=filechooser.showOpenDialog(stage); try { this.pereco.initFromFile(file); } catch
       * (FileNotFoundException e1) { e1.printStackTrace(); } this.pereco.ArrangeEcos();
    ...
    this.addObjBut.setOnAction(e -> {
      ...
      PersonalApp a =
          PersonalAppFactory.getInstance(name, company, version, functionality, businessarea);
    ...
    this.remObjBut.setOnAction(e -> {
       ...
       * Set<PersonalApp> athset=athletesmap.keySet(); for(PersonalApp a:athset) {
       * if(a.getName().equals(name)) { deleath=a; break; } }
       */
    ...
    this.logdisBut.setOnAction(e -> {
      ...
      s.close();
      LogRecord lr =
          new LogRecord(Level.INFO, "Operation" + ",LogicalDistance," + inputtext + ",succeed");
      this.log.log(lr);
      this.logsaver.add(lr);
    });
    ...
    this.isLegalBut.setOnAction(e -> {
      ...
      boolean ret =
          this.pereco.synthensisLegal(this.orbitlist.get(index - 1), appname1, appname2, index);
  ...
  /**
   * Given an app use note and the max synthesis of apps on one circular orbit, calculating which
   * track should the app on.
   * 
  ...
  void draw() {
    ...
    nucircle.radiusProperty()
        .bind(circlepane.heightProperty().multiply((0.9 / 2) * (0.3 / maxradius)));
    ...
    /*
     * for(String name:appname) { System.out.println(name+"   "+this.nametoappmap.toString()); }
     */
