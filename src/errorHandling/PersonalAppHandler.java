...
public class PersonalAppHandler {
	...
	private String handleRelation(String str) {
		...
		int oneIndex=str.indexOf('<');int twoIndex=str.indexOf('>');
	...
	private String handleApp(String str) {
		...
		if(elements[1].matches("([[a-z][A-Z][0-9]]+)")==false) {
			ret="The company of app should be a label, "
					+ "only letter and number is allowed";
			System.out.println(ret);
			return ret;
		}
		...
		if(elements[4].startsWith("\"")==false) {
			ret="The business area of app should be a sentence, "
					+ "which starts with \"";
			System.out.println(ret);
			return ret;
		}
	...
	private String handleUser(String str) {
		...
		Scanner s=new Scanner(numstr);String username="a";
		try {
			username=s.next();
	...
	private String handlePeriod(String str) {
		...
		String numstr=str.substring(equalIndex+1, str.length());
		Scanner s=new Scanner(numstr);String period="a";
		try {
			period=s.next();
		}catch(NoSuchElementException e) {
		...
		if((period.equals("Day")||period.equals("Hour")
				||period.equals("Week")||period.equals("Month"))==false) {
