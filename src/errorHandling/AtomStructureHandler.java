...
public class AtomStructureHandler {
	...
	private String handleNumberOfElectron(String str) {
		str=str.trim();String strplus=str+';';
		Pattern pattern=Pattern.compile("(([[0-9]\\.]+)/([[0-9]\\.]+);)+");
		Matcher matcher=pattern.matcher(strplus);matcher.find();
		String message;
		try {
	...
	private String handleElementName(String str) {
		...
		Scanner s=new Scanner(numstr);String eleName ="ABC";
		try {
			eleName=s.next();
	...
	private String handleNumOfTracks(String str) {
		...
		String numstr=str.substring(equalIndex+1, str.length());
		Scanner s=new Scanner(numstr);int trackNum=4;
		try {
			trackNum=s.nextInt();
