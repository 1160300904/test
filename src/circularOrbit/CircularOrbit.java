...
import track.*;
/**
 * This is a class represents a multiple tracks system. In this system, there is One or zero centralObject 
 * which is surrounded by one or more tracks. And there are physicalObjects on the tracks. Each track has a
 * track number which is equal or greater than one.
 * This is a MUTABLE type.
 * @param  L  L represents the type of centralObject, it MUST be IMMUTABLE.
...
public interface CircularOrbit<L,E> {
	...
	/**
	 * create an empty CircularOrbit which has a CentralObject of type L, and physicalObjects of type E.
	...
	/**
	 * add a track of a specified radius  to the tracks of the circular system. If the track of the input
	 * radius is already in the system, then return false.
	...
	/**
	 * delete the input track in the orbit system.If the tracknum is out of range of [1,max track number]
	 * then the system is left unmodified.
	 * @param tracknum	the number of the track you want to delete.It must between the max number
	 ...
	 * @param tracknum				the number of track you want to add the physicalObject to.It must between the max number
	 * 								and the minimum number of the current track in orbit.
	 * @param theta					theta of the coordinate of the object you want to add.
	 * 								It MUST be in degree.
	 * @return						true if it succeeded in adding a physicalObject, false otherwise.
	 * 								If the track specified by tracknum is not in the system, then it returns
	 * 								false, and the system is left unmodified.
	 * 								If the object is already in the system, then it returns
	 * 								false, and the system is left unmodified.
	 * @throws IllegalArgumentException  if physicalObject is null, then throws IllegalArgumentException.
	 */
	 ...
	 * @param tracknum			from which you want to remove the object.It must between the max number
	 * 							and the minimum number of the current track in orbit.
	 * @return					true if it succeeded in removing the object.
	 * 							If the track specified by tracknum is not in the system, then it returns
	 * 							false, and the system is left unmodified.
	 * 							If the object does not in the system, then it returns
	 * 							false, and the system is left unmodified.
	 * @throws IllegalArgumentException  if physicalObject is null, then throws IllegalArgumentException.
	 */
	 ...
	 * @return						true if it succeeded in adding this relation, false otherwise.
	 * 								If either of these two objects doesn't exists in the system, then
	 * 								it returns false.
	 * @throws IllegalArgumentException  if physicalObject is null, then throws IllegalArgumentException.
	 */
	 ...
	 * @param sourtracknum	the number of the source track you want to move from.It must between the max 
	 ...
	 * 						number and the minimum number of the current track in orbit.
	 * @param theta		the coordinate where the object you want to put at.
	 * 					It MUST be in degree.
	 ...
	 * 						and the minimum number of the current track in orbit.
	 * @return				the number of objects on that track.If tracknum is out of bound, then it returns
	 * 						-1.
	 */
	public int NumOnTrack(int tracknum);
	
	/**
	 * Get the logical distance between two objects, not considering the connection between central objects
	 * with physical objects. which is the minimum number of edges that connects two objects on track. 
	 * if these two objects aren't connected, then return value is INF.
