...
public class ConcreteCircularOrbit<L, E> implements CircularOrbit<L, E> {
    ...
    /*
     * Abstract function: 1.centralobj is the central object of the circular orbit
     * system. 2.tracks is the tracks of the circular orbit system. 3.radiuses is
     * radiuses of the tracks. 4.obj are the sets of objects corresponds with
     * tracks. 5.thetas is a map maps objects to its theta degree. 6.ctr is
     * relations between objects and central object. 7.tr is relations between every
     * two objects.
     */
    /*
     * Rep Invariant: 1.the size of tracks and radiuses and obj must be the same.
    ...
    void checkRep() {
        ...
        /*
         * for(E object:thetaKeySet) { //assert object!=null; Double
         * d=thetas.get(object); //assert d!=null; Double dou=d; assert dou<360&&d>=0;
         * for(HashSet<E> sets:obj) { if(sets.contains(object)) { contains=true; } }
         * assert contains;contains=false; }
         */
        assert this.tracks.size() == this.radiuses.size();
        assert this.tracks.size() == this.obj.size();
        /*
         * for(HashSet<E> hs:obj) { for(E object1:hs) { assert
         * thetaKeySet.contains(object1); } } Map<E,Integer> count=new HashMap<>();
         * for(E obj1:thetaKeySet) { for(E obj2:thetaKeySet) { if(obj1.equals(obj2)) {
         * int c = count.getOrDefault(obj1, 0)+1; count.put(obj1, c); } } } for(Integer
         * in:count.values()) { assert in.intValue()==1; }
         */
    }
