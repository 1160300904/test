...
public class ConcreteCircularOrbit<L, E> implements CircularOrbit<L, E> {
	...
	/*Abstract function:
	 *	1.centralobj is the central object of the circular orbit system.
	 *	2.tracks is the tracks of the circular orbit system.
	 *	3.radiuses is radiuses of the tracks.
	 *	4.obj are the sets of objects corresponds with tracks.
	 *	5.thetas is a map maps objects to its theta degree.
	 *	6.ctr is relations between objects and central object.
	 *	7.tr is relations between every two objects.
	 */
	/*Rep Invariant:
	 * 	1.the size of tracks and radiuses and obj must be the same.
	 * 	2.elements in thetas must be the same with elements in obj.
	 */
	void checkRep() {
		for(Track t:tracks) {
			assert t!=null;
		...
		/*
		 * for(E object:thetaKeySet) {
			//assert object!=null;
			Double d=thetas.get(object);
			//assert d!=null;
			Double dou=d;
			assert dou<360&&d>=0;
			...
			assert contains;contains=false;
		}
		 */
		assert this.tracks.size()==this.radiuses.size();
		assert this.tracks.size()==this.obj.size();
		/*
		 * for(HashSet<E> hs:obj) {
			for(E object1:hs) {
				assert thetaKeySet.contains(object1);
			}
		}
		Map<E,Integer> count=new HashMap<>();
		for(E obj1:thetaKeySet) {
			for(E obj2:thetaKeySet) {
				if(obj1.equals(obj2)) {
	...
	public boolean transit(E object, double theta, int sourtracknum,int tartracknum) {
		...
		/*for now it will not get false in this addPhyToTrack function.
		 * if(result==false) {
			return false;
		}
		 */
	...
	public double getPhysicalDistance( E e1, E e2) {
		...
		theta1=Math.toRadians(theta1);theta2=Math.toRadians(theta2);
	...
	public int getLogicalDistance(E p1, E p2) {
		/*
		 * if(!this.thetas.containsKey(p1)||!this.thetas.containsKey(p2))
			return -1;
		 */
		if(p1.equals(p2))	return 0;
		LinkedList<E> pque=new LinkedList<>();LinkedList<Integer> preque=new LinkedList<>();
		List<E> visited=new ArrayList<>();E quefront=null;
		Set<E> targets=new HashSet<E>();int distance=0;int index=0;boolean visitedboo=false;
		//change visiting strategy?
		pque.add(p1);preque.add(-1);visited.add(p1);
		//此处判空要注意
			...
			for(E p:targets) {
				...
				if(visitedboo==false) {
					if(p.equals(p2)) {
						...
						pque.offer(p);preque.offer(index-1);
						//要注意加到未访问节点中
						visited.add(p);
					}
				}
